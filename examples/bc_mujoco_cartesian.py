"""
Used to train and load BC policy for simulated data in MujoCo with cartesian pose as action
"""
...
def load_trajectory_from_file(file_path):
    ...
    for line in lines:
        ...
        act_cart_list.append(np.array(pd.eval(cart_list), dtype="float32"))
...
# ----------------------------------------------------------------------------------
        ...
        if i == 0:
            ...
            delta = act_cart_list[i] - act_cart_list[i-1]
...
def load_predict_observation(file_path, policy):
    ...
    for line in lines:
        ...
        for cart_traj in data[2].strip(' ').strip('[').strip(']').split(' '):
            cart_list.append(pd.eval(float(cart_traj)))
        for joint_traj in data[3].strip(' ').strip('[').strip(']').split(' '):
            joint_list.append(float(joint_traj))
...
if __name__ == "__main__":
    ...
    save_policy(bc_trainer.policy,"../trained_policy/feeding_policy_mujoco_cart_action")

    # Load trained policy and sample observation data
    loaded_policy = bc.reconstruct_policy("../trained_policy/feeding_policy_mujoco_cart_action")
