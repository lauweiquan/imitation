...
def load_trajectory_from_file(file_path):
    ...
    for line in lines:
        ...
        # print(depth_image)
        for cart_traj in data[1].strip(' ').strip('[').strip(']').split(' '):
            cart_list.append(float(cart_traj))
        for joint_traj in data[2].strip(' ').strip('[').strip(']').split(' '):
...
bc_trainer.train(n_epochs=9)
save_policy(bc_trainer.policy,"../trained_policy/feeding_policy_mujoco")
# loaded_policy = bc.reconstruct_policy("../trained_policy/feeding_policy_mujoco")
